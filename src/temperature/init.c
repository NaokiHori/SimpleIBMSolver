...
static temperature_t *allocate(const domain_t * restrict domain){
  ...
  temperature->temp       = common_calloc(      TEMP_SIZE_0 *       TEMP_SIZE_1, sizeof(double));
  temperature->tempforcex = common_calloc(TEMPFORCEX_SIZE_0 * TEMPFORCEX_SIZE_1, sizeof(double));
  /* ! Runge-Kutta source terms are allocated ! 3 ! */
  temperature->srctempa = common_calloc(SRCTEMPA_SIZE_0 * SRCTEMPA_SIZE_1, sizeof(double));
  temperature->srctempb = common_calloc(SRCTEMPB_SIZE_0 * SRCTEMPB_SIZE_1, sizeof(double));
  temperature->srctempg = common_calloc(SRCTEMPG_SIZE_0 * SRCTEMPG_SIZE_1, sizeof(double));
  return temperature;
}
