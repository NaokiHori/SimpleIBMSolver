...
static list_t *all_memories = NULL;
...
/* search pointer */
static int search(list_t **node_root, list_t **node_curr, list_t **node_prev, const void *ptr){
  /*
...
static int kernel_list_attach(list_t **node_root, void *ptr){
  ...
  node_curr = my_calloc(1, sizeof(list_t));
...
static void *list_calloc(const size_t count, const size_t size){
  // allocate pointer
  void *ptr = my_calloc(count, size);
  kernel_list_attach(&all_memories, ptr);
  return ptr;
}

/* functions which decrease members */
static int kernel_list_detach(list_t **node_root, const void *ptr){
  list_t *node_prev = NULL;
  list_t *node_curr = NULL;
...
static void list_detach(const void *ptr){
  if(kernel_list_detach(&all_memories, ptr) != RETVAL_SUCCESS){
...
static int convert_endian(void *val, const size_t size){
  ...
  uint8_t *buf = list_calloc(n_bytes, sizeof(uint8_t));
...
static int load_magic_string(size_t *buf_size, FILE *fp){
  ...
  uint8_t *buf = list_calloc(nitems, sizeof(uint8_t));
  {
...
static int load_dict_and_padding(uint8_t **dict_and_padding, size_t *buf_size, size_t header_len, FILE *fp){
  ...
  *dict_and_padding = list_calloc(nitems, sizeof(uint8_t));
  {
...
static int extract_dict(char **dict, uint8_t *dict_and_padding, size_t header_len){
  ...
  bool *is_dict = list_calloc(e-s+1, sizeof(bool));
  {
  ...
  *dict = list_calloc(n_chars_dict+1, sizeof(char)); // + NUL
...
static int find_dict_value(const char key[], char **val, const char *dict){
  ...
  *val = list_calloc(n_chars_val+1, sizeof(char)); // + NUL
...
static int extract_shape(size_t *ndim, size_t **shape, const char *val){
  ...
  // 1. check number of dimension (ndim) to store shape
  {
    ...
    str = list_calloc(n_chars_val-2+1, sizeof(char));
  ...
  *shape = list_calloc(*ndim, sizeof(size_t));
  {
    char *str = NULL;
    // copy "val" to a buffer "str" after removing parentheses
    size_t n_chars_val = strlen(val);
...
static int extract_dtype(char **dtype, const char *val){
  ...
  REJECT_NULL(val, -1);
  size_t n_chars_val = strlen(val);
  *dtype = list_calloc(n_chars_val+1, sizeof(char));
...
static int create_descr_value(char **value, const char dtype[]){
  ...
  // +1 for NUL
  *value = list_calloc(n_chars+1, sizeof(char));
...
static int create_shape_value(char **value, const size_t ndim, const size_t *shape){
  ...
  n_digits = list_calloc(ndim, sizeof(size_t));
  for(size_t i = 0; i < ndim; i++){
  ...
  *value = list_calloc(n_chars, sizeof(char));
  for(size_t i = 0, offset = 1; i < ndim; i++){
    ...
    // + "," and "NUL"
    buf = list_calloc(n_digit+2, sizeof(char));
    // including ","
    {
...
static int create_dict(char **dict, size_t *n_dict, const size_t ndim, const size_t *shape, const char dtype[], const bool is_fortran_order){
  ...
  char **elements = list_calloc(n_elements_dict, sizeof(char *));
  ...
  *dict = list_calloc(n_chars_dict, sizeof(char));
  ...
  list_free(descr_value);
  list_free(fortran_order_value);
  list_free(shape_value);
  list_free(elements);
...
static int create_padding(uint8_t **padding, size_t *n_padding, uint8_t *major_version, const size_t n_dict){
  ...
  *padding = list_calloc(*n_padding, sizeof(uint8_t));
  // many ' 's: 0x20
...
static int create_header_len(uint8_t **header_len, size_t *n_header_len, const uint8_t major_version, const size_t n_dict, const size_t n_padding){
  ...
  if(major_version == 2){
    ...
    *header_len = list_calloc(*n_header_len, sizeof(uint8_t));
...
size_t simple_npyio_w_header(const size_t ndim, const size_t *shape, const char dtype[], const bool is_fortran_order, FILE *fp){
  ...
  size_t header_nitems;
  {
    ...
    size_t *sizes   = NULL;
    size_t *offsets = NULL;
    sizes   = list_calloc(n_datasets, sizeof(size_t));
    offsets = list_calloc(n_datasets, sizeof(size_t));
    ...
    header = list_calloc(header_nitems, sizeof(uint8_t));
