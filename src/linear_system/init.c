 ...
 * @param[in] glsizes : GLOBAL size of ARRAY (can differ for ux, uy, p)
...
linear_system_t *init_linear_system(sdecomp_t * restrict sdecomp, const int glsizes[NDIMS]){
  ...
  /* allocate pencils */
  {
    linear_system->x1pncl = common_calloc(x1pncl_isize * x1pncl_jsize, sizeof(double));
    linear_system->y1pncl = common_calloc(y1pncl_isize * y1pncl_jsize, sizeof(double));
  }
  ...
  {
    // x, y tri-diagonal matrices are NOT treated simultaneously
    // so allocate buffer for the largest matrix and share buffers
    const int size = max2(x1pncl_isize, y1pncl_jsize);
